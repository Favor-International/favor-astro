// GET /api/marketing/campaigns — campaign id -> description map for the marketing
// platform (attribution reporting: which campaign raised which dollar). One SKY
...
export const onRequestGet: PagesFunction<Env & { MARKETING_API_KEY?: string }> = async ({ request, env }) => {
  try {
    ...
    const cached = await env.BLACKBAUD_TOKENS.get('bb:cache:marketing-campaigns');
    ...
    const data = await bbJson<{ value?: Array<{ id: string; description?: string; category?: string }> }>(
      env,
      '/fundraising/v1/campaigns?limit=500',
    );
    ...
    await env.BLACKBAUD_TOKENS.put('bb:cache:marketing-campaigns', JSON.stringify(value), { expirationTtl: 3600 });
